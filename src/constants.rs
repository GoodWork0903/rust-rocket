use crate::error_response::error_responses::{
    ...
    WRONG_REQUEST_JSON,LOGIN_NOT_PERMIT_JSON,USERNAME_NOT_REGISTER_JSON,
    PASSWORD_NOT_CORRECT_JSON,USER_NOT_PERMINT_JSON,ALREADY_REGISTERED_USER_JSON
};
...
pub const LEN_PASSWORD: LenText = LenText { min: 5, max: 200 };
...
pub const LOGIN_NOT_PERMIT: (Status, Json<ErrorResponse>) =
    (ERROR_WRONG_REQUEST_STATUS, Json(LOGIN_NOT_PERMIT_JSON));

pub const USERNAME_NOT_REGISTER: (Status, Json<ErrorResponse>) =
    (ERROR_WRONG_REQUEST_STATUS, Json(USERNAME_NOT_REGISTER_JSON));

pub const PASSWORD_NOT_CORRECT: (Status, Json<ErrorResponse>) =
    (ERROR_WRONG_REQUEST_STATUS, Json(PASSWORD_NOT_CORRECT_JSON));
...
pub const ALREADY_REGISTERED_USER: (Status, Json<ErrorResponse>) = (
    ERROR_ALREADY_REGISTERED_STATUS,
    Json(ALREADY_REGISTERED_USER_JSON),
);

//admin ceate
pub const USER_NOT_PERMINT: (Status, Json<ErrorResponse>) =
    (ERROR_WEAK_PASSWORD_STATUS, Json(USER_NOT_PERMINT_JSON));
