...
pub fn get_valid_username_and_password(
    username: &str,
    password: &str,
    max_min_len_login: LenText,
    max_min_len_password: LenText,
) -> TypeValidTwoStr {
    if check_valid_text(username, max_min_len_login.max, max_min_len_login.min) {
...
pub fn valid_registration_data_user(
    ...
    match get_valid_username_and_password(
        ...
        TypeValidTwoStr::Ok =>  match get_valid_mail(&registration_request.email) {
            TypeValidMail::Ok => Ok,
